                    ...
                    if not years:
                        ...
                        if st.button("분석 실행하기"):
                            ...
                            if car_col in df_prod.columns:
                                ...
                                # 테이블 2 정렬 (OEM, CN 그룹 내에서의 Car code master 랭크)
                                table2['Rank'] = table2.groupby([hk_col, cn_col])['증감(Diff)'].rank(method='min', ascending=sort_ascending).astype(int)
                                
                                # OEM, CN으로 그룹화한 뒤, 그룹 내 Rank에 따라 정렬
                                table2 = table2.sort_values(by=[hk_col, cn_col, 'Rank'], ascending=[True, True, True])
                                
                                # 컬럼 순서 재배치 (Rank를 맨 앞으로)
                                cols = ['Rank'] + [c for c in table2.columns if c != 'Rank']
                                table2 = table2[cols]
                                # 인덱스 숨기기를 위해 Rank를 인덱스로 세팅
