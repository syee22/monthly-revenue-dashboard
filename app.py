...
def apply_common_styles(styler, apply_hkmc_color=False):
    ...
    if hasattr(styler, 'map_index'):
        ...
        for i in range(styler.index.nlevels):
            styler.applymap_index(highlight_total_index, axis=0, level=i)
...
def to_excel_multiple(df_dict):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        for sheet_name, df in df_dict.items():
            ...
            for i, col in enumerate(df.columns): worksheet.set_column(i+1, i+1, 15)
...
if uploaded_file:
    ...
    def build_summary_report(df_sub, index_cols, year, month, total_label="TTL (K.€)", index_names=None, sort_by_current_act=False):
        ...
        m_str, pm_str = MONTH_NAMES.get(month), MONTH_NAMES.get(prev_month)
        ...
        if len(index_cols) == 1:
            final_df.index = pd.MultiIndex.from_tuples([(x,) for x in final_df.index], names=current_index_names)
        else:
    ...
    def get_biz_report(df, biz_type, year, month):
        ...
        grand_row = pd.DataFrame([grand_total], index=pd.MultiIndex.from_tuples([('', 'TTL (K.€)', '', '')], names=['Cust. GR', 'Project', 'Con.', 'SOP']))
    ...
    def build_trend_report(df, end_year, end_month):
        ...
        for y, m in months:
            ...
            pivot_data[col_name] = df_act[(df_act['Year'] == y) & (df_act['Month'] == m)].groupby('Group 2')['Rev. (€)'].sum()
    ...
    if not df_biz.empty: 
        st.markdown(render_html_view(df_biz, "", apply_color=False, title="Direct & Commission Sales Revenue (K.€)"), unsafe_allow_html=True)
    ...
    for filter_key, display_name in BIZ_CONFIG.items():
        ...
        df_pe, _ = get_biz_report(raw_df, filter_key, selected_year, selected_month)
        if not df_pe.empty:
            st.markdown(render_biz_html_table(df_pe, apply_color=True, title=f"Sales Revenue: {display_name} (K.€)"), unsafe_allow_html=True)
            reports_to_download[f"{display_name}_Detailed"] = df_pe
