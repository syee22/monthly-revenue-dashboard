...
import numpy as np
import io
import re  # HTML 후처리를 위한 정규식 모듈 추가
...
if uploaded_file:
    ...
    years = sorted(raw_df['Year'].dropna().unique().astype(int))
    selected_year = st.sidebar.selectbox("연도", years, index=len(years)-1 if years else 0)
    selected_month = st.sidebar.selectbox("월", sorted(raw_df['Month'].dropna().unique().astype(int)))
    ...
    def render_html_table(df, phase_names):
       ...
       # Pandas 버전 이슈를 피하기 위해 CSS 스타일을 직접 스타일러에 주입합니다.
        ...
        # [핵심] 총합계 행의 'Cust.GR', 'Project', 'Con.' (1, 2, 3열)을 하나의 th로 병합(colspan)하여 Project 열 너비 최소화
