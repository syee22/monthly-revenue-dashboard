...
def apply_common_styles(styler, apply_hkmc_color=False, is_export=False):
    ...
    if apply_hkmc_color:
        def color_index_cells(v):
            val_str = str(v)
            if val_str in ['HYU', 'HKMC']: return f'background-color: #e6f2ff{imp};'
...
def to_excel_multiple(df_dict):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        for sheet_name, original_df in df_dict.items():
            ...
            apply_color = sheet_name in ["PE_HKMC_Summary", "Core_Biz_Grp1_Summary", "PE_Biz_Detailed", "Core_Biz", "Biz_Type_Summary"]
...
def get_core_biz_summary_report(df, year, month):
    ...
    # ★ 매출 보고서 로직 (원본 그대로 Group 1 필터링 적용 - GM 등 모든 브랜드 인식)
...
def get_biz_report(df, biz_type, year, month):
    ...
    for brand in brands:
        if brand == 'GM':
            ...
            # ★ 오류 수정 (Group 2 -> Group 1)
            brand_df = df_biz[df_biz['Group 1'] == brand].copy()
            prev_mask = (df['Business Type'].str.contains(biz_type, case=False, na=False)) & (df['Year'] == prev_year) & (df['Month'] == prev_month) & (df['Group 1'] == brand)
                ...
                if line.startswith("Sales Org."):
                    ...
                    if len(parts) > 2:
                        if parts[1].isdigit() and parts[2] == '': 
                            ...
                            # 날짜 컬럼을 동적으로 찾기 (인덱스 5번 이후에 나오는 날짜 형태 2개 추출)
