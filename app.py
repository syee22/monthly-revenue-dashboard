            ...
            for line in content.split('\n'):
                ...
                if line.startswith("Sales Org."):
                    ...
                    if len(parts) > 2:
                        if parts[1].isdigit() and parts[2] == '': current_customer = parts[1]
                        elif len(parts) >= 12 and parts[1] in ['YPR0', 'ZADD', 'YSPR']:
                            try: 
                            ...
                            # 날짜 컬럼을 동적으로 찾기 (인덱스 10번 이후에 나오는 날짜 형태 2개 추출)
                            date_candidates = [p.strip() for i, p in enumerate(parts) if i > 10 and re.match(r'^\d{1,4}[\.\-\/]\d{1,2}[\.\-\/]\d{1,4}$', p.strip())]
                            
                            if len(date_candidates) >= 2:
                                raw_from = date_candidates[0]
